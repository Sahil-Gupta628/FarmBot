...
import nltk
nltk.download(['punkt', 'wordnet', 'stopwords', 'averaged_perceptron_tagger', 'omw-1.4', 'brown',
              'vader_lexicon', 'maxent_ne_chunker', 'words', 'conll2000', 'movie_reviews', 'reuters'])
...
def predict_class(sentence, model):
    ...
    return_list = [{"intent": classes[k[0]],
                    "probability": str(k[1])} for k in results]
    return return_list
...
def translate_message(message, source_language='auto', target_language='en'):
    translated_message = GoogleTranslator(
        source=source_language, target=target_language).translate(message)
